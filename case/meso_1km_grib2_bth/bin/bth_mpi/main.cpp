...
//    return 0;
    ...
    for(auto count = my_rank; count < product_count; count += comm_size) {
        const auto &query = production_list[count];
        ...
        for(auto forecast_hour = 0; forecast_hour <= 24; forecast_hour++) {
            ...
            fmt::print(
                "[{rank}][{count:02d}/{total}][{name:<5}][{forecast_hour:02d}] file path: {file_path}\n",
                "rank"_a=my_rank, "count"_a=count, "total"_a=product_count, "name"_a=product_name, "forecast_hour"_a=forecast_hour, "file_path"_a=file_path);
